...
"""SOM with iterations over weight matrix"""
...
for x in range(1, weight_matrix.shape[0] + 1):
    for y in range(1, weight_matrix.shape[1] + 1):
        ...
        sump=weight_matrix[x-1,y-1,:][0]+weight_matrix[x-1,y-1,:][1]+weight_matrix[x-1,y-1,:][2]
        weights.append(sump)
        c=color_map(sump)
        ...
        all_coun,j=country_match(all_countries2, c)
        all_countries2 = all_coun
        if j != None:
          ax.text(x-0.4,y,countries[j+1],fontsize=8)
          new.append(country_names[j+1])
          new.append(c)
